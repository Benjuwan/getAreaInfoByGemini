...
// ※以前、このプロンプト内に `JSON`, `json` というキーワードを盛り込んでいたが、Gemini がそれらキーワードに過剰反応してJSON形式での出力に固執し、結果的に400エラーが発生する事態が起きた。そのため「構造化データ」というシンプルな表現に変更した。
...
> [!CAUTION]
> ※【対象エリアの周辺施設情報】とは、ユーザーが選択した【対象エリア】の周辺施設（例：学校や保育園、医療施設など）に関する情報（構造化データ）を指しますが、 **質問内容に含まれていない場合やデータが空の場合は無視** してください。
...
- 【対象エリアの周辺施設情報】（提供されたデータ）から得た情報と、外部知識から得た情報は明確に区別して記載すること
- 構造化データが空または関連情報が確認できない場合は、**「提供されたデータからは関連する情報が確認できませんでした」と明確にユーザーに伝える** こと
